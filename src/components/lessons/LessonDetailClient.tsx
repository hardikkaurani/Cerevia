...
import { ArrowLeft, CheckCircle2, Trophy, Loader2, Play, Code2, Layers, Award } from 'lucide-react';
...
export function LessonDetailClient({ lesson, initialCompleted }: LessonDetailClientProps) {
  ...
  // Tab State
  const [activeTab, setActiveTab] = useState<'notes' | 'practice' | 'resources' | 'certificate'>('notes');
  ...
  const contentMap: Record<
    string,
    { videoUrl: string; body: string; quiz: { question: string; options: string[]; correctIdx: number } }
  > = {
  ...
  return (
    <div className="flex flex-col gap-6 max-w-6xl mx-auto w-full pb-16 px-4 md:px-0">
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          <span className="flex items-center gap-1.5 text-xs font-bold text-amber-400 bg-amber-500/10 border border-amber-500/20 px-3 py-1 rounded-xl">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-2 space-y-6">
          ...
          <div className="space-y-3 bg-zinc-950 p-6 rounded-3xl border border-zinc-800 shadow-xl">
            <div className="flex items-center gap-3">
              <span className="text-[10px] font-extrabold uppercase tracking-widest text-blue-400 bg-blue-500/10 border border-blue-500/20 px-2.5 py-1 rounded-xl">
                {lesson.difficulty}
              </span>
          ...
          {activeTab === 'certificate' && <CertificatePreview isCompleted={isCompleted} />}

        </div>

        {/* Right Milestone Progress Sidebar */}
        <div className="space-y-6">
          <div className="rounded-3xl border border-zinc-800 bg-zinc-950 p-6 shadow-xl sticky top-24 space-y-6">
            <h3 className="font-extrabold text-base text-white">Lesson Milestone Progress</h3>
            ...
            <div className="space-y-4">
              <div className="flex items-start gap-3">
                {videoWatched || isCompleted ? (
                  ...
                  <div className="h-5 w-5 rounded-lg border border-zinc-800 bg-zinc-900 shrink-0 mt-0.5" />
                )}
                <div>
                  <p className="text-xs font-bold text-white">Watch Module Video</p>
                  <p className="text-xs text-zinc-400 font-normal">Play tutorial video to completion.</p>
                </div>
              </div>
              ...
              <div className="flex items-start gap-3">
                ...
                <div>
                  <p className="text-xs font-bold text-white">Pass Concept Quiz</p>
                  <p className="text-xs text-zinc-400 font-normal">Select correct option to verify topic.</p>
                </div>
              </div>
            </div>
            ...
            {isCompleted ? (
              <div className="flex flex-col gap-3">
                <div className="flex items-center justify-center gap-2 p-3.5 bg-emerald-500/10 border border-emerald-500/30 rounded-2xl text-emerald-400 text-xs font-bold">
                  <CheckCircle2 className="h-4 w-4" />
                  <span>Module Mastered & Saved</span>
                </div>
                <Link href="/lessons" className="w-full">
                  <button className="w-full h-11 rounded-2xl border border-zinc-800 bg-zinc-900 text-xs font-bold text-zinc-200 hover:bg-zinc-800">
                    Back to Syllabus
                  </button>
                </Link>
              </div>
              ...
              <button
                ...
                className="w-full h-11 rounded-2xl bg-white text-xs font-bold text-zinc-950 hover:bg-zinc-100 transition-all shadow-lg disabled:opacity-50 flex items-center justify-center gap-2"
              >
                {isSubmitting ? (
                  <>
