...
export function CertificatesGallery({
  ...
  const certificateList: CertificateItem[] = useMemo(() => {
    return [
      {
        id: 'cert-1',
        ...
        status: completedModules >= 4 ? 'unlocked' : 'locked',
        type: 'specialization',
      },
      {
        id: 'cert-2',
        ...
        status: completedModules >= 3 ? 'unlocked' : 'locked',
        type: 'specialization',
      },
      {
        id: 'cert-3',
        ...
        status: completedModules >= 2 ? 'unlocked' : 'locked',
        type: 'course',
      },
      {
        id: 'cert-4',
  ...
  const unlockedCount = useMemo(() => certificateList.filter((c) => c.status === 'unlocked').length, [certificateList]);
  const specializationCount = useMemo(() => certificateList.filter((c) => c.type === 'specialization').length, [certificateList]);
  const lockedCount = useMemo(() => certificateList.filter((c) => c.status === 'locked').length, [certificateList]);
  ...
  }, [certificateList, activeTab]);

  return (
    <div className="space-y-4">
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
        <div>
          <h2 className="text-xl font-extrabold text-slate-900 dark:text-white tracking-tight">Verified Certificates & Specializations</h2>
          <p className="text-xs text-slate-500 dark:text-zinc-400 font-medium">Industry-recognized credentials awarded upon passing Cerevia proctored evaluations.</p>
        </div>
      ...
      {/* Grid of Certificates / Clean Empty State */}
      {filteredCerts.length === 0 ? (
        <div className="p-12 text-center border border-dashed border-zinc-200 dark:border-zinc-800 rounded-3xl text-slate-500 dark:text-zinc-400 font-medium bg-white dark:bg-zinc-900/40">
          No certificates yet. Complete your first course to earn one.
        </div>
      ) : (
                ...
                <div className="flex items-center justify-end gap-2 pt-1">
                  {isUnlocked ? (
                    ...
                    >
                      ...
                      <span>Locked (Complete Coursework)</span>
                    </button>
                  )}
                </div>
              </div>
